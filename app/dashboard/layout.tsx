...
export default function Layout({ children } : { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        setPendingData(requestDocs.filter((doc) => {return doc.status === 'pending'})
            .sort((a: any, b: any) => {return a.collection_date - b.collection_date}))
        ...
        setCompletedData(requestDocs.filter((doc) => {return doc.status === 'completed' || doc.status === 'rejected' || doc.status === 'cancelled'})
            .sort((a: any, b: any) => {return b.collection_date - a.collection_date}))
        clearDocs()
        setIsLoading(false)
        })
        ...
        const unsubscribeTransactions = onSnapshot(transactionsCollection, (transactions) => {
            ...
            setTransactionsData(transactionsDocs.sort((a: any, b: any) => {return b.transactionDate - a.transactionDate}))
            clearDocs()
            setIsLoading(false)
        })
        ...
        const unsubscribeVouchers = onSnapshot(vouchersCollection, (vouchers) => {
            ...
            setVouchersData(vouchersDocs.sort((a: any, b: any) => {return b.voucherCreatedOn - a.voucherCreatedOn}))
            clearDocs()
            setIsLoading(false)
        })
