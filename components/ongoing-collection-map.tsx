...
export default function OngoingCollectionMap({ children, ...props } : React.HTMLAttributes<HTMLDivElement>) {
    const apiKey = `${process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY}`
    
    const { ongoingCoordinates, mapLocation, panMapLocation, centered, setCentered } = useContext(DashboardContext)
    ...
    return (
        <div {...props}>
           <APIProvider apiKey={apiKey}>
                ...
                >
                    { ongoingCoordinates ? ongoingCoordinates.map((marker, index) => {
