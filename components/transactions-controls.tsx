...
export default function TransactionsControls({ buttonActive, setButtonActive, rowSelection, mapShown, setMapShown } : CollectionControlsType ) {
    ...
    async function deleteRows(rowSelection: object) {
        ...
        try {
            await rowIds.forEach((row) => {
                const docRef = doc(db, 'ecollector_transactions', `${row}`)
                deleteDoc(docRef)
            })
                ...
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>Delete selected rows</DialogTitle>
                            <DialogDescription>
                                Deleted transactions can&apos;t be restored. Are you sure you want to <span >DELETE</span> the selected rows? 
                            </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                        <DialogClose asChild>
